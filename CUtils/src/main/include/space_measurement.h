 ...
 */
mem_size_t cuSpaceConsumptionConvert(mem_size_t from, space_unit_enum_t unit);

/**
 * like ::cuSpaceConsumptionConvert but returns a double, not an integer
 *
 * @param[in] from the space amount to convert
 * @param[in] unit the unit measure you would like to convert \c from to
 * @return the amount of memory in \c unit
 */
double cuSpaceConsumptionToDouble(mem_size_t from, space_unit_enum_t unit);
 ...
 */
mem_size_t cuSpaceConsumptionGetTotalVirtualMemory();
 ...
 */
mem_size_t cuSpaceConsumptionGetUsedVirtualMemory();
 ...
 */
mem_size_t cuSpaceConsumptionGetCurrentProcessUsedVirtualMemory();

/**
 * @return the total physical memory available on the system
 */
mem_size_t cuSpaceConsumptionGetTotalPhysicalMemory();

/**
 * @return the total physical memory used right now
 */
mem_size_t cuSpaceConsumptionGetPhysicalMemoryUsed();

/**
 * @return the total physical memory used by the current process right now
 */
mem_size_t cuSpaceConsumptionGetCurrentProcessPhysicalMemoryUsed();
...
#define _INVOKE_SPACE_FUNCTION(sme)																\
	(sme == SME_USED_VIRTUAL ? cuSpaceConsumptionGetUsedVirtualMemory() : (										\
		sme == SME_PROCESS_USED_VIRTUAL ? cuSpaceConsumptionGetCurrentProcessUsedVirtualMemory() : (			\
			sme == SME_USED_PHYSICAL ? cuSpaceConsumptionGetPhysicalMemoryUsed() : (								\
				sme == SME_PROCESS_USED_PHYSICAL ? cuSpaceConsumptionGetCurrentProcessPhysicalMemoryUsed() : 	\
					cuSpaceConsumptionGetCurrentProcessUsedVirtualMemory()										\
				)																				\
			)																					\
		)																						\
	)																							\
 ...
 * @code
 *  void* p = NULL;
 * 	CU_SPACE_PROFILE_CODE(spaceOccupied, SME_PROCESS_USED_VIRTUAL, SUE_BYTE) {
 * 		p = malloc(3 * 1024 * sizeof(char));
 * 	}
 * 	if (p != NULL) {
 * 		free(p);
 * 	}
 * 	printf("the instructions within CU_SPACE_PROFILE_CODE occupied %llu bytes\n.", spaceOccupied);
 * @endcode
 *
 ...
 */
#define CU_SPACE_PROFILE_CODE(spaceVar, _sme, _unit) \
	double spaceVar; \
		...
		for (space_unit_enum_t UV(unit) = _unit; UV(loop);) \
			for (space_measurement_enum_t UV(sme) = _sme; UV(loop); ) \
				for ( \
					...
					spaceVar = cuSpaceConsumptionToDouble(UV(endSpace) - UV(startSpace), UV(unit)) \
				) \
