...
/**
 * Represents the type of the function we need to feed to ::addAppender
 *
 * The typedef defines what exactly is an appender.
 *
 * An appender is whatever function that, given:
 ...
 *
 * @param[in] cmds a list of commands to execute before the logging
 * @param[in] logLevel the log level to use
 * @param[in] bufferSize size of the buffer @c buffer
 * @param[inout] buffer the buffer to use to store the log entry
 ...
 *
 * ```
 * binfo("The list is %s", (cuListBufferString(l, log_buffer, default_bufferstring), log_buffer));
 * ```
 *
...
#	pragma message "CRTICAL LOG TURNED ON"
#elif QUICK_LOG <= __LOG_ERROR
#	pragma message "ERROR LOG TURNED ON"
#elif QUICK_LOG <= __LOG_WARNING
#	pragma message "WARNING LOG TURNED ON"
#elif QUICK_LOG <= __LOG_INFO
#	pragma message "INFO LOG TURNED ON"
#elif QUICK_LOG <= __LOG_FINE
#	pragma message "FINE LOG TURNED ON"
#elif QUICK_LOG <= __LOG_FINER
#	pragma message "FINER LOG TURNED ON"
#elif QUICK_LOG <= __LOG_FINEST
#	pragma message "FINEST LOG TURNED ON"
#elif QUICK_LOG <= __LOG_DEBUG
#	pragma message "DEBUG LOG TURNED ON"
#else
#	pragma message "ALL LOGGING DISABLED"
#endif
